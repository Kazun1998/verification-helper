...
def _python_list_depending_files(path: pathlib.Path, basedir: pathlib.Path) -> List[pathlib.Path]:
    ...
    try:
        ...
        future = executor.submit(importlab.graph.ImportGraph.create, env, [str(path)], True)
        if platform.uname().system == 'Windows':
            timeout = 500.0  # 1.0 sec causes timeout on CI using Windows (オリジナルは 5.0 sec)
        else:
            timeout = 100.0   # (オリジナルは 1.0 sec)
